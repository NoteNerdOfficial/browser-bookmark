...
function readArcSidebarFile(path) {
...
function renderNode(items, node, depth) {
  var _a;
...
${indent}<DL><p>
` + kids.map((kid) => renderNode(items, kid, depth + 1)).join("") + `${indent}</DL><p>
`;
  }
  return `${indent}<DT><A HREF="${escapeHtml((_a = node.url) != null ? _a : "")}">${escapeHtml(node.title)}</A>
`;
}
function exportToNetscapeHtml(items) {
  ...
  if (pinnedItems.length > 0) {
    body += "  <DT><H3>Pinned</H3>\n  <DL><p>\n" + pinnedItems.map((node) => renderNode(items, node, 2)).join("") + "  </DL><p>\n";
  }
...
var BookmarkListView = class extends import_obsidian6.ItemView {
  ...
  showBookmarkMenu(node, evt) {
    ...
    menu.addItem(
      (item) => item.setTitle("Open").setIcon("external-link").onClick(() => this.openDefault(node))
    );
    menu.addItem(
    ...
    menu.addItem(
      (item) => item.setTitle("Open in system browser").setIcon("globe").onClick(() => {
        var _a;
    ...
    menu.addSeparator();
    menu.addItem(
      (item) => item.setTitle("Copy link").setIcon("copy").onClick(() => {
        var _a;
        void this.copyToClipboard((_a = node.url) != null ? _a : "");
      })
    );
    menu.addItem(
      (item) => item.setTitle("Copy as Markdown link").setIcon("copy").onClick(() => {
        var _a;
        void this.copyToClipboard(`[${node.title}](${(_a = node.url) != null ? _a : ""})`);
      })
    );
    menu.addSeparator();
    menu.addItem(
      (item) => item.setTitle("Always open externally").setIcon("globe").setChecked(Boolean(node.openExternally)).onClick(() => void this.store.toggleOpenExternally(node.id))
    );
    menu.addSeparator();
  ...
  openImportMenu(evt) {
    ...
    menu.addSeparator();
    menu.addItem(
      (item) => item.setTitle("Export bookmarks").setIcon("file-down").onClick(() => this.exportBookmarks())
    );
    menu.showAtMouseEvent(evt);
  }
  /**
